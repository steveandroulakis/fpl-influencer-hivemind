...
Usage:
    ...
    uv run ./youtube-titles/fpl_video_picker.py --max-per-channel 10 --out results.json
"""
...
def load_channels_from_file(file_path: str, logger: logging.Logger) -> list[str]:
...
Use --single-channel for parallel processing in shell scripts.
    ...
    parser.add_argument(
        "--channels-config",
        default="./youtube-titles/channels.json",
        help="Path to channels.json config file (default: ./youtube-titles/channels.json)",
    )
    parser.add_argument(
        "--max-per-channel",
        type=int,
